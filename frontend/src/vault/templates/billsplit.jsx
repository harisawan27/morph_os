...
export default function BillSplitter() {
  ...
  return (
    <div className="h-full bg-[#0a0a0a] text-white flex flex-col p-5 gap-4 overflow-y-auto morph-scrollbar">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white/4 border border-white/8 focus:border-white/20 rounded-2xl pl-9 pr-4 py-3 text-xl text-white/85 placeholder-white/18 outline-none transition-colors"
          />
        </div>
      </div>
      ...
      {/* Tip */}
      <div>
        <label className="text-[10px] uppercase tracking-widest text-white/25 mb-2 block items-center gap-1.5">
          <Percent size={10} /> Tip
        </label>
        <div className="flex flex-wrap gap-1.5 mb-2">
          {TIP_OPTIONS.map(t => (
            <button key={t} onClick={() => { setTip(t); setUseCustom(false); }}
              className={`px-3 py-1.5 rounded-xl text-xs border transition-all ${!useCustom && tip === t ? 'bg-purple-600/20 border-purple-500/30 text-purple-300' : 'bg-white/3 border-white/[0.07] text-white/40 hover:text-white/70'}`}>
              {t === 0 ? 'No tip' : `${t}%`}
            </button>
          ))}
          <button onClick={() => setUseCustom(true)}
            className={`px-3 py-1.5 rounded-xl text-xs border transition-all ${useCustom ? 'bg-purple-600/20 border-purple-500/30 text-purple-300' : 'bg-white/3 border-white/[0.07] text-white/40 hover:text-white/70'}`}>
            Custom
          </button>
        </div>
        {useCustom && (
          <div className="relative">
            <input type="number" value={customTip} onChange={e => setCustomTip(e.target.value)}
              ...
              className="w-32 bg-white/3 border border-white/8 focus:border-white/15 rounded-xl px-3 py-2 text-sm text-white/70 placeholder-white/18 outline-none" />
          ...
          <AnimatePresence>
            {names.map((name, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }}
                className="flex items-center gap-2 bg-white/3 border border-white/6 rounded-xl px-3 py-2">
      ...
      {bill > 0 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          className="bg-white/3 border border-white/[0.07] rounded-2xl p-4 mt-auto shrink-0">
          <div className="space-y-2 mb-3">
            ...
            <div className="flex justify-between text-sm border-t border-white/6 pt-2 text-white/70">
