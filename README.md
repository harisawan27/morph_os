...
| :--- | :--- | :--- |
| **Multi-agent / ADK** | Custom multi-agent orchestrator utilizing a custom Planner/Executor pattern (Brain ➔ Executor agents). *Note: The Google Agent Development Kit (ADK) library is NOT used directly. Instead, orchestration is custom-built on top of the native `google-genai` SDK.* | [llm_pipeline.py](file:///f:/morph_os/backend/llm_pipeline.py) (`brain_plan_ui`, `execute_plan`, `builder_generate_react`, `builder_edit_react`, `chat_respond`, `search_web`). |
...
| **Security** | End-to-end user identity protection and server isolation. Includes custom signed NextAuth HS256 JWT validation, secure cookie session handling, CORS origin restrictions, programmatic input sanitization, and API secret isolation. | [route.ts](file:///f:/morph_os/frontend/src/app/api/auth/%5B...nextauth%5D/route.ts) (OAuth flow, custom jose HS256 JWT sign), [auth.py](file:///f:/morph_os/backend/auth.py) (`get_current_user` JWT validation), [main.py](file:///f:/morph_os/backend/main.py) (CORS `ALLOWED_ORIGINS` & custom `_check_rate_limit`), [ArtifactRenderer.tsx](file:///f:/morph_os/frontend/components/ArtifactRenderer.tsx) (regex ES6 import sanitization). |
| **Deployability** | Full multi-service containerization, build cache optimizations, and production environment mapping for Hugging Face Spaces & Vercel. | [Dockerfile](file:///f:/morph_os/Dockerfile) (port `7860`, uvicorn CMD), [README_HF.md](file:///f:/morph_os/backend/README_HF.md) (Hugging Face Docker Space setup instructions), [globals.css](file:///f:/morph_os/frontend/src/app/globals.css) (PWA assets). |
| **Agent Skills / Tool Use** | Live web grounding tool usage, programmatic rule checking Critic tool, structured file analysis, and browser state-syncing APIs. | [llm_pipeline.py](file:///f:/morph_os/backend/llm_pipeline.py#L569) (`search_web` calling `google_search`), [llm_pipeline.py](file:///f:/morph_os/backend/llm_pipeline.py#L1025) (`analyze_file` parser), [llm_pipeline.py](file:///f:/morph_os/backend/llm_pipeline.py#L784) (`_critic_check` programmatic tool), [ArtifactRenderer.tsx](file:///f:/morph_os/frontend/components/ArtifactRenderer.tsx) (`morphSaveState` / `morphLoadState`). |
